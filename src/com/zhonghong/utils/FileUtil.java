...
//			Log.i(TAG, "isDeviceMounted " + device);
	 ...
	 * @return
	 */
	public static boolean isDeviceMounted2(Context context, String device){
		StorageManager storageManager = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
		StorageVolume[] storageVolume = storageManager.getVolumeList();

		for (int i = 0; i < storageVolume.length; i++) {
			StorageVolume sVolume = storageVolume[i];
			if (sVolume.getPath().equals(device)
					&& sVolume.getState().equalsIgnoreCase("mounted")) {
				return true;
			}
		}
		Log.i(TAG, "isDevice not Mounted " + device);
